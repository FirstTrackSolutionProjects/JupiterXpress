...
import approveInternationalRequestShipmentService from '../services/shipmentServices/internationalShipmentServices/approveInternationalRequestShipmentService';
import rejectInternationalRequestShipmentService from '../services/shipmentServices/internationalShipmentServices/rejectInternationalRequestShipmentService';
...
// Updated ManageForm (copied & adapted from UpdateOrderInternational.jsx)
    ...
    try{
      ...
      for (let i = 0; i < formPayload.dockets.length; i++) {
        for (let j = 0; j < formPayload.items.length; j++) {
          if (parseInt(formPayload.items[j].box_no) == i+1){
            docketFlag = 1
          }
        }
        if (docketFlag == 0){
      ...
      let itemFlag = 0
      for (let i = 0; i < formPayload.items.length; i++) {
        for (let j = 0; j < formPayload.dockets.length; j++) {
          if (formPayload.items[i].box_no == formPayload.dockets[j].box_no){
            itemFlag = 1
          }
        }
        if (itemFlag == 0){
...
const Listing = ({ step, setStep }) => {
  ...
  // Pass refresh to Card so it can trigger a refresh after actions
      ...
      <div className={`w-full p-4 flex flex-col items-center space-y-6 ${step == 0 ? "" : "hidden"}`}> 
        ...
        <div className="w-full">
          {shipments.map((shipment, index) => (
            <Card key={index} shipment={shipment} onRefresh={handleRefresh} />
          ))}
        </div>
      </div>
    </>
